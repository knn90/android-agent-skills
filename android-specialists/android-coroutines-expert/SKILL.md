...
- **Captured `var`, not value:** a coroutine closure captures a **reference** to a `var`, so a value reassigned after `launch` is seen by the coroutine — a shared `var` mutated across iterations (`var i = 0; while (…) { i++; launch { use(i) } }`) races. Snapshot into a `val` before launching. (A Kotlin `for`-loop variable is already a fresh `val` per iteration — `for (i in xs) launch { use(i) }` is safe; don't "fix" it.)
