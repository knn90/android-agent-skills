---
name: android-skill-consolidate
...
Keeps a consolidated specialist skill (Compose, coroutines, …) current by re-synthesizing it
...
## Inputs
- `target-skill` — the skill to (re)build, e.g. `android-compose-expert` or `android-code-review`. Its folder must hold a `SOURCES.yaml`. **If omitted, a selection menu is shown (Step 0).** Pass `all` to consolidate every one in turn.
...
> **hand-curated, not synthesized** (e.g. `android-resolve` — a workflow/orchestration skill, not
...
- **Otherwise, present a menu.** Discover consolidatable skills by scanning for any skill folder
  that contains a `SOURCES.yaml` — under `android-specialists/` (the specialist skills) **and**
  `android-skills/` (core skills that opt in). Ask via `AskUserQuestion` with one numbered option
  **per skill found**, plus an **"All"** option:
  ...
  `android-specialists/<name>/` or `android-skills/<name>/` makes it appear automatically, no edit here.
...
superseded APIs (e.g. pre-`StateFlow` LiveData-only, KAPT where KSP now applies)? → propose `status: retired` **with a reason**.
...
- `WebSearch` the domain using the `discovery.queries` seeds + the current year, AND check the
  `authority_anchors` (developer.android.com docs / Google I/O / Material Design guidelines) for guidance changes.
...
- **Precedence — the `primary: true` source wins.** The official source (Google /
  developer.android.com docs, Google I/O, Material Design guidelines — whichever the skill's
  `SOURCES.yaml` marks `primary: true`) is the **source of truth** and overrides community sources
  on any conflict, gated to the project's `min_sdk`. Community sources fill in practical patterns
  and mistake heuristics, not semantics.
- **Dedupe** overlapping advice; **flag** genuine conflicts in a "Contested" note (don't silently pick).
- **Organize by topic** appropriate to the domain (e.g. for Compose: state, composition, layout, performance, navigation, animation, accessibility, testing).
...
- **Header format** — one blockquote only: "**Generated skill** — original wording, consolidated by
  `android-skill-consolidate`; provenance, sources, and licenses live in `SOURCES.yaml`. Don't hand-edit —
...
## Modes
- **On-demand:** `android-skill-consolidate android-compose-expert` (add `--no-discover` for a quick same-sources refresh).
