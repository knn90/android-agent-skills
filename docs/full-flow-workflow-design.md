...
The generalized `android-skills/` suite already covers most of the flow. Every skill reads `.claude/android-profile.md`.
...
| Bootstrap | `android-project-init` | the `android-profile.md` itself |
...
2. **`android-execute` implements solo** — no parallel team engine (worktree dev agents + peer review + merge + validation gate).
...
### 3.1 New skill — `android-resolve` (orchestrator)

> Name: **`android-resolve`** (chosen — works for "resolve ticket" *and* free-form).

```
android-resolve <TICKET-ID | "free-form description">
...
```
0. Load .claude/android-profile.md  (missing → android-project-init)
1. Resolve input:
...
### 3.2 Extend the implementer — `android-execute --team N` (renamed from `android-execute`)

The implementer is **renamed `android-execute` → `android-execute`** and gains a team mode, honoring the suite principle *"one implementer."* Today Phase 3 is: direct edits, or delegate one subagent for big features. We add a third path:
...
The team protocol is large, so it lives in **`android-execute/references/team-execution.md`** (loaded only for `--team` runs), keeping the main SKILL.md readable. Protocol:
...
| deprecation | remove tests + mocks | remove implementation + DI | remove references (incl. XML layouts/manifest/generated bindings) |
...
|---|---|
| `./gradlew :app:assembleDebug` / `:app:testDebugUnitTest` (build + test gates) | **`verify_command`** (already the single source of truth) |
| a design-system module, logging wrapper, DI container, view-model base class, crash reporter, async framework | **`rules_file`** + existing `architecture`/`di`/`networking`/`crash_reporting` + android-execute & android-code-review profile-gated checks |
...
| reference check via `rg` + XML layout/manifest sweep; generated-binding / `@Parcelize` blockers | **universal Android** — bake into engine, not profile |
...
*Optional future ports (not v1):* `android-add-test` (runs tests 5× to catch flakiness) and `android-automation-test-identifier` (QA test tags) — flag as add-ons if you want deeper test rigor.
...
android-resolve ──INIT──► branch + plans_dir/{slug}/_status.md
      ...
      │
      ├─► android-code-review ─────► verdict (Critical must clear)
      │
...
|---|---|---|
| **1. Profile + scaffold** | Add `default_base_branch`, `pr_tool` to `android-profile.template.md`; create `android-resolve/` folder; add `android-execute/references/` | template parses; folders exist |
| **2. Team engine** | `android-execute/references/team-execution.md` (spawn→context→build→review→merge→validate, generalized prompts/roles/gates); wire `--team N` + `--solo` flags into `android-execute` Phase 3 | dry-read: every source-project hardcode replaced by a profile ref or documented default |
| **3. Orchestrator** | `android-resolve/SKILL.md` (full pipeline §3.1, gates, ticket fetch, branch logic, PR, report) | walk-through on a sample ticket id + a free-form string (no code run) |
...
| **5. Docs** | Update root `README.md` pipeline diagram to include `android-resolve`; link this design | README shows new entry point |
| **6. Dry-run** | Exercise the chain end-to-end on one real Android app with a filled profile (small ticket) | branch + scope.md + plan.md + PR produced; gates pass |
...
1. **Orchestrator name** — **`android-resolve`**.
2. **Engine placement** — **folded into the implementer**, which is **renamed `android-execute` → `android-execute`**; the `--team N` protocol lives in `android-execute/references/team-execution.md`.
3. **Dedicated test skills** — **deferred**. v1 testing = TDD in team mode + `verify_command` gate + review coverage check. (`android-add-test` 5× flake runs, `android-automation-test-identifier` QA tags are later add-ons.)
...
- **Emulator/test contention** when several dev agents run `verify_command` concurrently — keep verification at the *integrate* step (post-merge), not per-dev, to serialize it.
