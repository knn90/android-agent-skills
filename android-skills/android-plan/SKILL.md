---
name: android-plan
description: "Create detailed, phased implementation plans for an Android project. Use for feature planning, refactors, ticket resolution. Does NOT implement — hands off to android-execute. Subcommands: archive, red-team, validate."
...
> Every phase is ONE end-to-end feature (model → service → state → UI → tests) that leaves the
...
| Need trade-off analysis | `android-brainstorm` |
...
working plan dir, written when `android-resolve` ran `android-scout` upstream — read it instead of
re-scouting. Otherwise gather it now: scope spanning 3+ modules → `android-scout`, else `Glob`/`Grep`.
Capture per file to modify: `path:line` · existing pattern to follow (DRY) · test file + pattern ·
DI keys touched · navigation routes/destinations touched · network operations touched (never edit
`generated_paths`) · localization keys + test tags needed.

### Step 3 — Research (skip if `--fast`)
`--hard`/`--two` → `android-research <topic>`. Default → lightweight `WebSearch` only if truly
needed. **Don't research patterns already in the codebase.**

### Step 4 — Solution design
Per approach: UI (Composable) / state ({state_type}) / navigation split · DI wiring ({di}) · network
operation + cache strategy ({networking}) · feature flag (`feature_flags`) + default-off
rollout · backwards-compat/migration · testing strategy (unit/screenshot/UI with test
tags) · rollback plan. `--two` → comparison table.
...
- **Dependency graph → order the slices.** Map what depends on what; order bottom-up — typically
  model/persistence → networking/service → DI wiring → state/store → UI → tests. Foundation slice first.
- **Contract first (team runs).** If slices will be built in parallel (complexity MEDIUM/HIGH),
  shared contracts (interfaces, DI keys, API surface) land in the foundation slice before slices fork.
...
Hard multi-layer reasoning (data-flow, state transitions, races) → `android-sequential-thinking`.
...
├── phase-2-{slice-a}.md       # vertical slice: model→service→state→UI→tests
...
complexity: MEDIUM         # LOW | MEDIUM | HIGH — drives android-execute's dev count
...
## Testing Strategy    Unit / Screenshot / UI (test tags needed)
...
**Red team** (`--hard`/`--two` or subcommand) — `android-plan red-team <plan-dir>` spawns an
...
**Validate** (optional) — `android-plan validate <plan-dir>` via `AskUserQuestion`: ACs mapped to
phases? feature flag confirmed with PM? backend schema timing confirmed? design signed off?
localization strings provided? test tags reviewed? QA bandwidth? Save to `validation.md`.

### Step 8 — Execute handoff (do NOT auto-invoke)
```
Plan ready: {plans_dir}/{plan-dir}/plan.md
- Implement now            → android-execute {plans_dir}/{plan-dir}/plan.md
- Grill open decisions     → android-grill {plans_dir}/{plan-dir}
- Adversarial review first → android-plan red-team <plan-dir>
- Validate with team       → android-plan validate <plan-dir>
```
(When `android-resolve` drives the flow, it runs this handoff for you.)
