...
Loaded by `android-execute` for `--team N` runs. You are the **Team Lead**: spawn a dev team in
...
Generalized — **every project fact comes from `.claude/android-profile.md`** (`source_roots`,
...
names, modules, or emulators.

## Inputs (set by the caller before loading this file)

| Var | From | Notes |
|---|---|---|
| `N` | `--team N` / `--devs N`, else auto per `android-execute` Phase 3 | `N == 1` → use the solo path in `android-execute` Phase 3, not this engine |
| `SLUG` | task / plan dir name | |
| `TASK_DIR` | `{plans_dir}/{SLUG}/` — holds `scope.md`, `plan.md`, `_status.md` | |
| `BASE` | **the branch the team builds on.** From `android-resolve` = the task branch `{type}/{SLUG}`. Standalone = the current branch (else `default_base_branch`). | every dev + integrate branch is cut from this — so the merge stays within the task's lineage |
...
```
You are {AGENT_NAME}, a senior Android engineer working in this repo ({source_roots}).
Authoritative conventions: read .claude/android-profile.md and {rules_file} first, and follow
them exactly (if android-profile.md is absent in your worktree — it is usually gitignored — read
the main checkout's copy: $(git rev-parse --path-format=absolute --git-common-dir)/../.claude/android-profile.md) — architecture {architecture}, state {state_type}, DI {di},
navigation {navigation}, networking {networking}, localization {localization},
test tags {test_tags}, feature flags {feature_flags}.
Never edit anything under {generated_paths}. Money = BigDecimal, never Double. No PII in logs.
...
|---|---|---|---|
| **feature** | a vertical slice (e.g. Composable + view-model) **+ its tests** | another slice (e.g. repository + networking) **+ its tests** | integration / DI wiring + edge cases **+ its tests** |
| **deprecation** | remove tests + mocks for target | remove implementation + DI registrations | remove references (imports, XML layouts, manifest entries, reflection) |
...
### Rules
- Follow .claude/android-profile.md + {rules_file}. Never touch {generated_paths}.
...
Checklist (same lens as `android-code-review` Stage 2):
...
- [ ] Blocking work off the main thread (correct `Dispatchers`); coroutines launched in `viewModelScope` (cancellation-aware); no leaked scopes/collectors
- [ ] State via `{state_type}`; DI via `{di}`; navigation per `{navigation}` (no `NavController` held by a VM)
- [ ] No hardcoded user-facing strings (if `localization` != none); nothing under `{generated_paths}`
- [ ] Money = `BigDecimal`; no PII in logs (esp. `high_rigor_domains`)
- [ ] No commented-out/dead code or debug logs
...
> You are an adversarial edge-case reviewer for an Android feature. Read `{TASK_DIR}/plan.md` +
...
> - state-transition holes (loading → loaded / empty / error), emission ordering / stale state after `suspend`
> - boundary / off-by-one; ordering & race conditions where slices interact
> - money: rounding, sign, precision (BigDecimal) — if in `high_rigor_domains`
...
`android-code-review` adversarial pass (in `android-execute` Phase 5) stays as the final gate.
...
Serialized here (never per-dev) to avoid emulator/test contention:
```
cd .worktrees/{SLUG}/integrate
{verify_command}
```
- **`verify_command` unset/empty** → build-only (`./gradlew assembleDebug`); state it explicitly. Never claim tests passed.
...
```
The **review gate runs in `android-execute` Phase 5** on the integrate branch — do not review
