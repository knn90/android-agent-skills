...
**sequences and gates** — the real work is done by `wayfinder` (planning) and the other `android-*`
skills (implementation): never re-do what a sub-skill already does, never skip a gate.
Project-agnostic: every fact comes from `.claude/android-profile.md` — never hardcode app specifics.
...
| A described change spanning spec→code→PR | Implement an existing spec/tracer-bullet ticket → `android-execute <path>` |
| | Trade-off discussion → `android-brainstorm` · review only → `android-code-review` |

**Sizing** (referenced below):
- **trivial** — single file, < ~20 lines → skip this skill; edit directly (or `android-execute --fast`).
...
5. SCOUT           android-scout → scope.md
6. IMPLEMENT       android-execute (owns the verify + review gates)
...
### Step 5 — Scout
Invoke `android-scout {target}` (target = main component/feature from the spec or input ticket).
...
android-execute {spec path | tracer-bullet ticket} [--team N | --solo]
```
- Pass `--devs`/`--solo` through if given; otherwise `android-execute` auto-picks from the spec's scope.
