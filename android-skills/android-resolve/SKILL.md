---
name: android-resolve
description: "Resolve a ticket or a free-form request end-to-end for an Android project — the single front door from input to open PR. Use when given a ticket id or told to 'resolve/ship/work on' something."
argument-hint: "[TICKET-ID | \"description\"] [--devs N | --solo] [--base BRANCH] [--skip-approval] [--no-grill] [--no-pr]"
---

# Resolve — Android end-to-end workflow

One command takes a **ticket or free-form context** all the way to an open PR. This skill only
**sequences and gates** — the real work is done by the other `android-*` skills: never re-do what a
sub-skill already does, never skip a gate. Project-agnostic: every fact comes from
`.claude/android-profile.md` — never hardcode app specifics.

## Step 0 — Load profile

Read `.claude/android-profile.md`: `source_roots`, `plans_dir`, `high_rigor_domains`,
...
|---|---|
| A ticket id to take to PR | Just find files → `android-scout` |
| "Resolve / ship / work on X" end-to-end | Just a plan, no code → `android-plan` |
| A described change spanning scope→plan→code→PR | Implement an existing plan → `android-execute <plan-path>` |
| | Trade-off discussion → `android-brainstorm` · review only → `android-code-review` |

For a trivial one-file fix, skip this — edit directly (or `android-execute --fast`).
...
android-resolve "add pull-to-refresh on the orders list"
android-resolve ABC-123 --devs 3 --base release/1.2
android-resolve ABC-123 --solo --skip-approval
android-resolve "fix crash on empty cart" --no-pr
```

## Argument parsing
- **TICKET-ID** — matches `ticket_pattern` → fetch via `ticket_fetch` (Step 1).
- **"free-form"** — used as the task description (no ticket fetch).
- `--devs N` / `--solo` — passed through to `android-execute`.
...
3. SCOUT           android-scout → scope.md
4. PLAN            android-plan  → plan.md
4.5 GRILL          android-grill → hardened plan.md + grill.md
    GATE           ──► APPROVAL on the hardened plan
5. IMPLEMENT       android-execute (owns the verify + review gates)
...
### Step 3 — Scout
Invoke `android-scout {target}` (target = main component/feature from the ticket, or the description).
Write its map to `{plans_dir}/{slug}/scope.md`. Skip only for a truly trivial single-file change.

### Step 4 — Plan
Invoke `android-plan` for this task (point it at the Step 3 `scope.md` so it reuses that map instead
...
Unless skipped (`--no-grill` / `--skip-approval`), invoke `android-grill {plans_dir}/{slug}` to
...
- Complexity: {…} (dev count auto-picked by android-execute)   Files: {n}   Feature flag: {name|n/a}   HIGH-RIGOR: {yes/no}
- Key changes: {3-5 bullets}
Reply: approved · revise: {feedback} · abort
```
`revise:` → re-run `android-plan` with feedback. `abort` → set `_status.md` CANCELLED, stop.
...
android-execute {plans_dir}/{slug}/plan.md [--team N | --solo]
```
- Pass `--devs`/`--solo` through if given; otherwise `android-execute` auto-picks from the plan's `complexity`.
- **Team mode** returns `{slug}/integrate` (already verified + reviewed inside `android-execute`).
...
- If `android-execute` returns BLOCKED (e.g. team merge conflict — it reports files/owners) →
  surface its report and stop.

### Step 6 — Commit
`android-execute` commits granularly via `/commit`. If anything is still uncommitted (status/doc
...
2. **Verify** — owned by `android-execute`.
3. **Review** — owned by `android-execute`.

- **DO NOT** implement code yourself — `android-execute` is the only implementer.
